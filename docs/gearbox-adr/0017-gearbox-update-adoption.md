# ADR-0018: gearbox-update 入库——写侧回流工具转正,护栏写成契约

- Date: 2026-07-19
- 溯源: 回流自 [gearbox ADR-0017](https://github.com/real-stanyan/gearbox/blob/master/docs/adr/0017-gearbox-update-adoption.md)（gearbox-update 工具同步）
- Status: accepted

## Context

`scaffold-update`(node,Z Code 2026-07-19 产出)是写侧下游同步工具:扫描下游缺失的上游 ADR → 撞号自动重编号 → 单遍全文替换交叉引用 → supersede 状态联动 → 每个 ADR 一个 commit → push 到下游新分支 → 出报告。与只读的 gearbox-version(ADR-0017)互补。
