# ADR-0017: gearbox-version 入库——B-3.5 本地速查工具转正

- Date: 2026-07-19
- 溯源: 回流自 [gearbox ADR-0016](https://github.com/real-stanyan/gearbox/blob/master/docs/adr/0016-gearbox-version-adoption.md)（gearbox-update 工具同步）
- Status: accepted

## Context

`scaffold-version`(bash,查下游落后上游几个 ADR)在 dryrun#40 实战中被引用,但居无定所:不在版本控制、没走流程(ADR-0013 说 B-3.5 = 手动 + 本地脚本,"届时另开 ADR"),且纯文件名编号对比有已知误报——下游自有 ADR 占同号(dryrun 0011 = puppeteer ≠ 上游 0011 = subagent-system)会被报成"已同步"。缺口挂在 issue #30,维护者拍板选 b(入库 + 修误报),顺应 ADR-0016 改名 gearbox-version。
