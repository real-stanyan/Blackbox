# ADR-0023: gearbox-install——开局安装从 README 手工步骤变成工具

- Date: 2026-07-19
- 溯源: 回流自 [gearbox ADR-0022](https://github.com/real-stanyan/gearbox/blob/master/docs/adr/0022-gearbox-install-tool.md)（gearbox-update 工具同步,上游 sha256:b2d326f83f62）
- Status: accepted
- 关联: ADR-0016/0017(工具家族第三件)、ADR-0022(hash 戳记随装即打)
...
- 手拷的 ADR 无溯源无戳记 → 全量 legacy,ADR-0022 的漂移检测对新项目失效
...
3. **ADR 全量拷贝 + hash 戳记**:每个上游 ADR 加溯源行 + sha256 戳记(ADR-0022 契约),新项目 day one 即可被 `gearbox-version` 检测漂移;自带溯源字段的(date-cli 时代 0004/0005/0006)不重复加,照 gearbox-update 同规则计 legacy
